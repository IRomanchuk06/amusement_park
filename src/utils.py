...
def get_age(prompt):
    return get_valid_input(prompt, int, lambda x: 0 <= x <= 120, "Age must be between 0 and 120.")

def get_num_greater_0(prompt):
    return get_valid_input(prompt, float, lambda x: x > 0, "Number must be greater than 0.")

def get_height(prompt):
    return get_valid_input(prompt, float, lambda x: 0 <= x <= 300, "Height must be between 0 and 300.")
